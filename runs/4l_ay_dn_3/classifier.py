...
conv1_1 = tf.layers.conv2d(normalized, 30, (3,3), padding='same', activation=tf.nn.relu)
pool1 = tf.layers.max_pooling2d(conv1_1, (2,2), (2,2))

conv2_1 = tf.layers.conv2d(pool1, 40, (3,3), padding='same', activation=tf.nn.relu)
conv2_drop = tf.layers.dropout(conv2_1, rate=0.1, training=is_training)
pool2 = tf.layers.max_pooling2d(conv2_drop, (2,2), (2,2))

conv3 = tf.layers.conv2d(pool2, 50, (3,3), padding='same', activation=tf.nn.relu)
conv3_drop = tf.layers.dropout(conv3, rate=0.1, training=is_training)
pool3 = tf.layers.max_pooling2d(conv3_drop, (2,2), (2,2))

conv4 = tf.layers.conv2d(pool3, 60, (3,3), padding='same', activation=tf.nn.relu)
pool4 = tf.layers.max_pooling2d(conv4, (2,2), (2,2))

final_pool_layer = tf.layers.max_pooling2d(pool4, (3,7), (1,1))

conv_flat = tf.reshape(final_pool_layer, [-1, 60])
flat_drop = tf.layers.dropout(conv_flat, rate=0.1, training=is_training)
#dense = tf.layers.dense(conv_flat, units=50, activation=tf.nn.relu)
logits = tf.layers.dense(flat_drop, units=NUM_CLASSES, name='logits')
...
with tf.Session() as sess:
	...
	for epoch in range(EPOCHS):
		...
		for image_batch, label_batch in get_train_batches(IMAGES_PER_BATCH):
			_, curr_accuracy, curr_loss, curr_conf = sess.run([train_op, accuracy_op, cross_entropy_loss, conf_mat], feed_dict = {input_layer: image_batch, y: label_batch, is_training: True})
		...
		for image_batch, label_batch in get_test_batches(IMAGES_PER_BATCH):
			curr_accuracy, curr_conf = sess.run([accuracy_op, conf_mat], feed_dict = {input_layer: image_batch, y: label_batch, is_training: False})
			samples += len(label_batch)
