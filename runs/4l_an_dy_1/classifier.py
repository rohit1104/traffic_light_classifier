...
conv1_1 = tf.layers.conv2d(normalized, 30, (3,3), padding='same')
pool1 = tf.layers.max_pooling2d(conv1_1, (2,2), (2,2))

conv2_1 = tf.layers.conv2d(pool1, 40, (3,3), padding='same')
pool2 = tf.layers.max_pooling2d(conv2_1, (2,2), (2,2))

conv3 = tf.layers.conv2d(pool2, 50, (3,3), padding='same')
pool3 = tf.layers.max_pooling2d(conv3, (2,2), (2,2))

conv4 = tf.layers.conv2d(pool3, 60, (3,3), padding='same')
...
dense = tf.layers.dense(conv_flat, units=100)
logits = tf.layers.dense(dense, units=NUM_CLASSES, name='logits')
...
with tf.Session() as sess:
	...
	for epoch in range(EPOCHS):
		...
		if test_accuracy >= 0.99 and train_accuracy >= 0.99:
			break
