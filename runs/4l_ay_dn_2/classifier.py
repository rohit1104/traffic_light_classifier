...
conv2_1 = tf.layers.conv2d(pool1, 35, (3,3), padding='same', activation=tf.nn.relu)
pool2 = tf.layers.max_pooling2d(conv2_1, (2,2), (2,2))

conv3 = tf.layers.conv2d(pool2, 40, (3,3), padding='same', activation=tf.nn.relu)
pool3 = tf.layers.max_pooling2d(conv3, (2,2), (2,2))

conv4 = tf.layers.conv2d(pool3, 50, (3,3), padding='same', activation=tf.nn.relu)
pool4 = tf.layers.max_pooling2d(conv4, (2,2), (2,2))

final_pool_layer = tf.layers.max_pooling2d(pool4, (3,7), (1,1))

conv_flat = tf.reshape(final_pool_layer, [-1, 50])
